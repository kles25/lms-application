...
import { enrolleesColumns } from "./DataTableSource";
...
const EnrolleesData = () => {
    ...
    useEffect(() => {
        const fetchEnrolleesWithCourseNames = async () => {
            try {
                const enrolleesSnapshot = await getDocs(collection(db, 'enrollees'));
                const enrolleesData = [];

                for (const docRef of enrolleesSnapshot.docs) {
                    const enrollee = docRef.data();
                    const courseId = enrollee.course; // Assuming courseId is the field containing the course ID

                    // Fetch the course document using the courseId
                    const courseDoc = await getDoc(doc(db, 'courses', courseId));
                    const courseData = courseDoc.data();

                    const enrolleeWithCourseName = {
                        id: docRef.id,
                        ...enrollee,
                        courseName: courseData.courseName // Assuming 'courseName' is the field in the courses collection containing the course name
                    };

                    enrolleesData.push(enrolleeWithCourseName);
                }

                // Now enrolleesData contains enrollees' data with their associated course names
                setData(enrolleesData);
            } catch (error) {
                console.error('Error fetching enrollees:', error);
            }
        };

        fetchEnrolleesWithCourseNames();

        const unsub = onSnapshot(
            collection(db, "enrollees"),
            (snapShot) => {
                let list = [];
    ...
    const confirmDelete = async () => {
        try {
            await deleteDoc(doc(db, 'enrollees', selectedItemId));
    ...
    return (
        <div className="datatable">
            ...
            <DataGrid style={{ color: "#fffffe", fontWeight: "300", backgroundColor: "#071c30e6" }}
                className="datagrid"
                rows={data}
                columns={enrolleesColumns.concat(actionColumn)}
                pageSize={9}
